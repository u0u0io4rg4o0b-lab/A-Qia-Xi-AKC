...
// 1) 取得一次性 nonce
exports.siweNonce = (0, https_1.onRequest)(withCors(async (req, res) => {
...
// 2) 驗證簽名
exports.siweVerify = (0, https_1.onRequest)(withCors(async (req, res) => {
    res.set('x-akc-siwe-version', AKC_SIWE_VERSION);
    res.set('Access-Control-Expose-Headers', 'x-akc-siwe-version');
    ...
    try {
        ...
        if (!message || !signature) {
            res.status(400).json({ success: false, error: '缺少 message 或 signature' });
            return;
        }
        ...
        const { success, data, error } = await msg.verify({ signature, nonce, time: new Date().toISOString() }, // ★ 先不傳 domain，避免套件直接拋 Domain mismatch
        { suppressExceptions: true });
        console.log('[siweVerify] verify.success =', success, ' error =', error?.type || error);
        if (!success) {
            res.status(401).json({ success: false, error: error?.type || 'invalid_signature' });
            return;
        }
...
exports.siweLogout = (0, https_1.onRequest)(withCors(async (req, res) => {
    // 清 Cookie、清 session
    setCookie(res, '__session', '', { maxAge: 0 });
    setCookie(res, 'signature', '', { maxAge: 0 });
    res.status(204).send('');
